...
public class AbstractCmGroupArchivesMemberHisImpl implements IQueryCountBusiModel, IQueryDataBusiModel
{

	private transient static Log log = LogFactory.getLog(AbstractCmGroupArchivesMemberHisImpl.class);
	...
	public Parameter buildQueryCondition(DataContainer[] conditionDCs) throws Exception 
	{
		...
		{
			condition.append(IBoGroupArchivesMemberHisValue.S_DoneDate).append(" >=to_date('"+conditionDCs[0].getAsString("DONE_START_DATE").substring(0, 10)+"' ,'yyyy-mm-dd')");
		}
		if(!StringUtils.isBlank(conditionDCs[0].getAsString("DONE_END_DATE")))
		{
			if(condition.length()>0)
				condition.append(" AND ").append(IBoGroupArchivesMemberHisValue.S_DoneDate).append(" <=to_date('"+conditionDCs[0].getAsString("DONE_END_DATE")+"' ,'yyyy-mm-dd')");
			else
				condition.append(IBoGroupArchivesMemberHisValue.S_DoneDate).append(" <=to_date('"+conditionDCs[0].getAsString("DONE_START_DATE").substring(0, 10)+"' ,'yyyy-mm-dd')");
		}
