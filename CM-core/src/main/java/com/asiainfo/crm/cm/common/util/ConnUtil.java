...
public class ConnUtil {
	...
	private static final String DATA_SOURCE_PARTNER = "partner";
	 ...
	 */
	public static Connection getReadOnlyPartnerConn() throws Exception {
		return ServiceManager.getSession().getConnection(getRealDataSource(DATA_SOURCE_PARTNER, ""));
	}
	 ...
	 */
	public static Connection getWriteAblePartnerConn() throws Exception {
		return ServiceManager.getSession().getNewConnection(getRealDataSource(DATA_SOURCE_PARTNER, ""));
	}
	...
	public static String getDeleteTmpTableSql(String tempTable, String field, int modNum, int modVal) {
		...
		if (modNum < 1 || modVal < 0 || StringUtils.isBlank(field)) {
			...
			if (modNum == 1 && modVal == 0) {
				...
				sql.append(" where mod(").append(field.toUpperCase()).append(",").append(modNum).append(")=").append(modVal);
			}
		}
