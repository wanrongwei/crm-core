...
import com.asiainfo.crm.cm.common.util.CmLunar;
import com.asiainfo.crm.cm.common.util.FtpFileNamingUtil;

public class GroupContactBrithdayRemindLunar extends AbstractFpProcessImpl {

	private static transient Log log = LogFactory.getLog(GroupContactBrithdayRemindLunar.class);
	...
	public String getFileName() throws Exception {
		String fileName = FtpFileNamingUtil.getTodayFileName("group_cont_birth_remind_lunar", ".dat", "yyyyMMdd");
		return fileName;
	}
	...
	public void process() throws Exception {
		...
		// 当前农历日期
		String date = sdf.format(CmLunar.getLunarCalendar(sdf.format(new Date())));
		//7天后的日期
		String nextDate = sdf.format(TimeUtil.addOrMinusDays(TimeUtil.getTimstampByString(date, "yyyy-MM-dd").getTime(), 7));
		...
		try {
			...
			sql.append("AND TO_CHAR(T.BIRTHDAY, 'MMdd') > TO_CHAR(TO_DATE('").append(date).append("','yyyy-MM-dd'), 'MMdd') ");
			sql.append("AND TO_CHAR(T.BIRTHDAY, 'MMdd') <= TO_CHAR(TO_DATE('").append(nextDate).append("','yyyy-MM-dd'), 'MMdd') ");
